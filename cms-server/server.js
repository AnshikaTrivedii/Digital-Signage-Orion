    ...
    try {
        ...
        if (stateCount === 0) {
            await GlobalState.create({ 
                tickerNews: "BREAKING: ANTIGRAVITY OS v2.5 DEPLOYED WITH MONGODB. • ALL EDGE NODES SYNCHRONIZED.",
                style: "Neon",
                color: "#00e5ff",
                speed: "Normal"
            });
...
// Fetch Active Data for Displays
const getActiveData = async () => {
    try {
        ...
        if (activeList) {
            ...
            // Default Hardcoded FailSafe
            thePlaylist = [
                { id: "fs1", type: "video", url: "https://www.w3schools.com/html/mov_bbb.mp4", duration: 8000 },
                { id: "fs2", type: "image", url: "https://images.unsplash.com/photo-1557683316-973673baf926?auto=format&fit=crop&q=80&w=3200", duration: 5000 },
            ];
        }
        return { playlist: thePlaylist, ticker: theTicker };
    } catch (e) {
        return { 
            playlist: [{ id: "err1", type: "image", url: "https://images.unsplash.com/photo-1557683316-973673baf926?auto=format&fit=crop&q=80&w=3200", duration: 8000 }], 
...
app.post('/api/upload', upload.single('media'), async (req, res) => {
    ...
    try {
        ...
        if (req.body.pushLive === 'true') {
            const newItem = { id: newAsset._id.toString(), assetId: newAsset._id, url: newAsset.url, type: newAsset.type, duration: newAsset.duration };
