...
# Local/Render: fall back to DATABASE_URL when DIRECT_URL is unset. Despite the
...
# AWS production runs migrations in CodeBuild before instance refresh.
if [[ "${SKIP_STARTUP_MIGRATE:-}" != "true" ]]; then
  ...
  if [[ "${ORION_SEED_DEMO_DATA:-}" == "true" ]]; then
